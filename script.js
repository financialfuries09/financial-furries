...
function addArticleStyles(){
  ...
  s.id="ff-published-styles";

  s.textContent=`
  #ff-news,#ff-published{margin:40px 0}
  .ff-news-grid,.ff-published-grid{
    display:grid;
  ...
  try{
    ...
    section.innerHTML=`
      <div class="section-head">
        <div>
          <span class="kicker">DAILY FINANCIAL NEWS</span>
          <h2>What's happening in finance</h2>
        </div>
        <span class="updated">Updated daily</span>
      </div>

      <div class="ff-news-grid">
        ${items.slice(0,12).map(a=>`
          <a class="ff-news-card"
             href="${esc(a.url)}"
             target="_blank"
            ...
            <div class="ff-news-source">
              ${esc(a.source||"Source")} · Read original →
            </div>
      ...
      <div class="ff-news-note">
        Headlines and links are provided for news discovery.
        Full articles remain with their original publishers.
      </div>
    `;

  }catch(e){
    console.log("Daily news unavailable",e);
  }
}

function renderSharedArticles(articles){
  sharedArticles=Array.isArray(articles)?articles:[];
  ...
  section.innerHTML=`
    <div class="section-head">
      <div>
        <span class="kicker">PUBLISHED BY FINANCIAL FURRIES</span>
        <h2>Latest articles</h2>
      </div>
    </div>

    <div class="ff-published-grid">
      ${sharedArticles.slice().reverse().map((a,i)=>`
  ...
  section.querySelectorAll("[data-ff-index]").forEach(b=>{
    b.addEventListener("click",()=>{
      openReader(
        sharedArticles.slice().reverse()[Number(b.dataset.ffIndex)]
      );
    });
  });
}
...
function openReader(article){
  ...
  if(!reader){
    ...
    reader.innerHTML=`
      <div class="ff-reader-box">
        <button class="ff-reader-close">×</button>
        <div id="ff-reader-inner"></div>
      </div>
    `;

    document.body.appendChild(reader);

    reader.querySelector(".ff-reader-close").onclick=()=>{
  ...
  document.getElementById("ff-reader-inner").innerHTML=`
    ...
    ${article.image?`
      <img
        ...
        alt=""
        style="max-width:100%;border-radius:12px;margin:18px 0"
      >
    `:""}

    <p>
      <strong>${esc(article.excerpt||"")}</strong>
    </p>
    ...
    <div class="ff-reader-disclaimer">
      <strong>Financial Disclaimer:</strong>
      ${esc(FINANCIAL_DISCLAIMER)}
    </div>
  `;
...
async function loadSharedArticles(){
  try{
    const r=await fetch("articles.json?"+Date.now(),{cache:"no-store"});

    if(!r.ok)throw new Error("articles.json unavailable");

    const data=await r.json();

    renderSharedArticles(data.articles||[]);

  }catch(e){
    console.log("Shared articles unavailable",e);
  }
}
...
function setupSearch(){
  ...
  input.addEventListener("input",()=>{
    ...
    const pub=sharedArticles.map(a=>[
      a.category||"Article",
      a.title||"",
      a.excerpt||""
    ]);

    const matches=defaultStories
      .concat(pub)
      .filter(s=>s.join(" ").toLowerCase().includes(q));

    results.innerHTML=matches.length
      ?matches.map(s=>`
        <div class="result">
          <b>${esc(s[0].toUpperCase())}</b>
          <h3>${esc(s[1])}</h3>
        </div>
      `).join("")
...
function setupNewsletter(){
  const f=document.getElementById("newsletter");
  ...
  f.addEventListener("submit",e=>{
    ...
    m.textContent=
      "You're on the list. (Demo form — connect an email service before launch.)";
...
async function loadMediaNews(){
  ...
  section.innerHTML = `
    <div class="section-head">
      ...
      <span class="updated">Financial sources</span>
    </div>
    ...
    <div class="ff-news-grid">
      ${mediaSources.map(source => `
        <a
          ...
          rel="noopener noreferrer"
        >
          <div class="ff-news-meta">FINANCIAL MEDIA</div>

          <h3>${source.name}</h3>

          <p>
            Read the latest financial and business news
            directly from ${source.name}.
          </p>
    ...
    <div class="ff-news-note">
      News links take you to the original publisher.
      Financial Furries does not reproduce full third-party articles.
    </div>
  `;
